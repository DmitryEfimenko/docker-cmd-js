...
describe('Cmd()', function () {
    ...
    it('run()', function (done) {
        cmd.run("docker-machine status " + const_1.machineName).then(function (status) {
            expect(status).toBe("Running\n");
            done();
        }, function (err) { done.fail(err); });
    });
    it('run() without new lines', function (done) {
        cmd.run("docker-machine status " + const_1.machineName, true).then(function (status) {
            expect(status).toBe('Running');
            done();
        }, function (err) { done.fail(err); });
    });
    it('runSync()', function () {
        var status = cmd.runSync("docker-machine status " + const_1.machineName);
